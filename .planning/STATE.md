...
status: executing
stopped_at: Plan 04-01 execute-complete — src/locations.py + 7 normalizer helpers wired (salary verbatim + normalize_location) + renderer salary coalesce/truncate; ready for Plan 04-02 (FILT-07 US-only filter)
last_updated: "2026-06-08T17:58:13Z"
progress:
  total_phases: 4
  completed_phases: 3
  total_plans: 12
...
**Plan:** 04-01 complete — `src/locations.py` (NEW, 151 lines) exporting pure `normalize_location()` (Remote-variant collapse → canonical `Remote (US)` / `Remote (non-US)`; non-Remote strings verbatim per D-02b) + `is_us_location()` (8-rule classifier per D-02a: empty bias → Remote canonical → US state code → US country token → US city → non-US substring → fallback bias; 51 US state codes + 30 US cities + 45 non-US tokens; all regex compiled at module load; <1ms/call). All 7 `_normalize_<adapter>` helpers in `src/normalizer.py` now populate `Posting.salary` from source-specific access path per D-01 (Greenhouse `metadata[].value` matching `_GREENHOUSE_SALARY_METADATA_NAMES` frozenset; Lever `salaryRange.text → salary`; Ashby `compensation.compensationTierSummary` defensive against None; SR/Workday hard-coded empty with inline-comment rationale; Apple `postingPay.payRange.text → salaryRange → homeOffice`; Playwright `raw['salary']` best-effort) AND route location through `normalize_location()` (Apple composes locations[].name list FIRST then normalizes the composed string). New `_extract_greenhouse_salary(raw)` helper extracted for unit-testability. `src/renderer.py` salary cell now flows through `_coalesce_salary()` (D-01a — empty/None/`Competitive`/`DOE`/`TBD`/`TBC`/`Not disclosed`/`N/A`/`null`/`Negotiable`/`Depends on experience`/`to be determined`/`—` → `—`) → `_truncate_cell(limit=80)` (D-01b — single-codepoint U+2026 ellipsis after rstrip) → existing `escape_markdown_cell` (NORM-07 unchanged). 124 net new tests (56 locations + 20 normalizer + 23 renderer + 25 parametrized expansions); 499 cumulative passing. ADP-15 re-proven a SEVENTH time (zero edits to src/adapters/*; `git diff --name-only HEAD~7 -- src/adapters/` = 0). NORM-02 + NORM-03 ready to mark complete in REQUIREMENTS.md. `is_us_location` is exported, tested, and ready for Plan 04-02 FILT-07 to wrap in `is_us_location_acceptable(posting)`. Commits: d4235db Task 1 RED, 160c9f8 Task 1 GREEN, fb547f1 Task 2 RED, d7b9e1b Task 2 GREEN, fc9bf83 Task 3 RED, 8550ee5 Task 3 GREEN.
...
- **Phases complete:** 0/4 (Phase 1 + Phase 2 + Phase 3 all execute-complete; Phase 4 Plan 04-01 of 3 complete, awaiting verification)
- **Requirements mapped:** 71/71 (100%)
- **Requirements validated:** 70/71 (56 in Phase 1 + ADP-04/05/06 in Plan 02-01 + ADP-07 in Plan 02-02 + ADP-08 + FILT-03 in Plan 02-03 + ADP-09 + ADP-10 in Plan 03-02 + SEC-01 + SEC-02 + SEC-04 + SEC-06 in Plan 03-03 + NORM-02 + NORM-03 in Plan 04-01)
- **Plans complete:** 10/12 (3/3 Phase 01 + 3/3 Phase 02 + 3/3 Phase 03 + 1/3 Phase 04 — Plan 04-01: ~50min, 3 TDD tasks, 2 created + 4 modified files, 124 net new tests / 499 cumulative)
...
**Last session:** 2026-06-08T17:58:13Z
**Last action:** Completed Plan 04-01 (3 TDD tasks, 6 commits: d4235db Task 1 RED, 160c9f8 Task 1 GREEN, fb547f1 Task 2 RED, d7b9e1b Task 2 GREEN, fc9bf83 Task 3 RED, 8550ee5 Task 3 GREEN). 499 cumulative tests pass; 124 net new (56 locations + 20 normalizer + 23 renderer + 25 parametrized expansions). ADP-15 invariant preserved (git diff src/adapters/ count = 0 over the 7-commit window including SUMMARY commit). Ruff clean. NORM-02 + NORM-03 ready to close in REQUIREMENTS.md. is_us_location classifier exported and ready for Plan 04-02 FILT-07 consumption.
**Previously:** Completed Plan 03-02 (2 tasks committed: 7202dc0 Task 1 = src/adapters/playwright_fallback.py PlaywrightAdapter with XHR-intercept-first via page.expect_response + DOM-selector fallback via wait_for_selector + selectolax + playwright-stealth on by default (D-04) + 60s navigation timeout (D-05) + trace=off-in-prod with SCRAPER_DEBUG_TRACE escape hatch (D-06) + `pw:<host>:<id>` dedup keys with sha256(url)[:16] fallback (Pitfall 9) + tests/fixtures/anthropic_sample.{json,html} + tests/test_playwright_adapter.py 22 tests; ac6f40f Task 2 = registry imports PlaywrightAdapter + appends LAST in ADAPTERS per D-01c (now 7 adapters total) + normalizer gains _normalize_playwright + _read_playwright_description + 'playwright' key in _DISPATCH + 5 normalizer tests + 5 registry tests + updated test_new_adapter_can_be_added to insert before catch-all + rewrote 2 NoAdapterFound assertions as PlaywrightAdapter dispatch assertions); 348/348 cumulative tests passing (316 baseline + 22 adapter + 5 normalizer + 5 registry net = +32); ruff check src/ tests/ clean; SUMMARY.md written at `.planning/phases/03-playwright-fallback-credential-workflow/03-02-SUMMARY.md`. PlaywrightAdapter dispatch path proven: `CompanyConfig(name='anthropic', url='https://www.anthropic.com/careers')` → `PlaywrightAdapter`; specific-adapter precedence preserved (Greenhouse URL still → GreenhouseAdapter). ADP-09 + ADP-10 closed. ADP-14/15 open-closed re-proven a fifth time: zero edits to any of src/adapters/{greenhouse,lever,ashby,smartrecruiters,workday,apple}.py.
**Stopped at:** Plan 04-01 execute-complete — locations.py + normalizer salary/location wiring + renderer salary coalesce/truncate ready; NORM-02 + NORM-03 closeable; ready for Plan 04-02 (FILT-07 US-only filter wrapping is_us_location)
**Resume file:** None
...
*Plan 04-01 complete: 2026-06-08 — src/locations.py (NEW, 151 lines) exporting normalize_location() (Remote-variant collapse per D-02 to canonical Remote (US) / Remote (non-US); non-Remote verbatim per D-02b) + is_us_location() (8-rule classifier per D-02a; 51 US state codes + 30 US tech-hub cities + 45 non-US tokens; all regex compiled at module load; <1ms/call). All 7 _normalize_<adapter> helpers in src/normalizer.py now populate Posting.salary verbatim from source-specific path per D-01 (Greenhouse via dedicated _extract_greenhouse_salary metadata-list scanner; Lever salaryRange.text → salary fallback; Ashby compensation.compensationTierSummary defensive against None; SR/Workday hard-coded empty with rationale; Apple postingPay.payRange.text → salaryRange → homeOffice; Playwright raw['salary'] best-effort) AND route location through normalize_location() (Apple composes locations[].name list FIRST then normalizes the composed string). src/renderer.py salary cell now flows through _coalesce_salary() (D-01a — empty/None/Competitive/DOE/TBD/TBC/Not disclosed/N/A/null/Negotiable/Depends on experience/to be determined/em-dash → '—') → _truncate_cell(limit=80) (D-01b — single-codepoint U+2026 ellipsis after rstrip) → existing escape_markdown_cell (NORM-07 unchanged). 124 net new tests (56 locations + 20 normalizer + 23 renderer + 25 parametrized expansions); 499 cumulative passing. ADP-15 re-proven a SEVENTH time (zero edits to src/adapters/*; `git diff --name-only HEAD~7 -- src/adapters/` = 0). NORM-02 + NORM-03 ready to mark complete in REQUIREMENTS.md. is_us_location classifier exported, tested, and ready for Plan 04-02 FILT-07 to wrap in is_us_location_acceptable(posting). Commits: d4235db Task 1 RED, 160c9f8 Task 1 GREEN, fb547f1 Task 2 RED, d7b9e1b Task 2 GREEN, fc9bf83 Task 3 RED, 8550ee5 Task 3 GREEN.*
